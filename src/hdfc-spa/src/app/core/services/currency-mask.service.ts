...
export class CurrencyMaskService {
  ...
  transform(value: string, allowNegative = false, decimalPrecision: number = 2) {
    ...
    fraction =
      decimalPrecision > 0
        ? this.decimalSeparator + (fraction + '000000').substring(0, 3)
        : '';

    integer = integer.replace(/\B(?=(\d{2})+(?!\d))/g, this.thousandsSeparator);
    ...
    var replacedstring = integer;
    replacedstring = replacedstring.replace(/\,/g, '');
    if (replacedstring.length > 3) {
      var lastThree = replacedstring.substring(replacedstring.length - 3);
      var otherNumbers = replacedstring.substring(0, replacedstring.length - 3);
      if (otherNumbers != '') {
        lastThree = ',' + lastThree;
        var res =
          otherNumbers.replace(/\B(?=(\d{2})+(?!\d))/g, ',') + lastThree + fraction;
      }
    } else {
      res = replacedstring;
    }
